...
export class AddNode extends ReactiveNode<{ num: number, num2: number }, { num: number }> {
    ...
    async builder(node: RNode): Promise<void> {
        ...
        inp1.addControl(new NumControl((val: number) => this.liveValues.get(node.id)?.set(val), 'num', false, i18n(["addIn"]) || "Addend"))
        inp2.addControl(new NumControl((val: number) => this.liveValues2.get(node.id)?.set(val), 'num2', false, i18n(["addIn"]) || "Addend"))
