...
export class MultNode extends ReactiveNode<{ num: number, num2: number }, { num: number }> {
    ...
    async builder(node: RNode): Promise<void> {
        const inp1 = new Rete.Input('num', i18n(["multIn"]) || "Factor", SocketTypes.numSocket().valSocket);
        const inp2 = new Rete.Input('num2', i18n(["multIn"]) || "Factor", SocketTypes.numSocket().valSocket);
        ...
        inp1.addControl(new NumControl((val: number) => this.liveValues.get(node.id)?.set(val), 'num', false, i18n(["mutlIn"]) || "Factor"))
        inp2.addControl(new NumControl((val: number) => this.liveValues2.get(node.id)?.set(val), 'num2', false, i18n(["multIn"]) || "Factor"))
