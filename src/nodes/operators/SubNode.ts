...
export class SubNode extends ReactiveNode<{ num: number, num2: number }, { num: number }> {
    ...
    async builder(node: RNode): Promise<void> {
        ...
        const inp2 = new Rete.Input('num2', i18n(["subIn2"]) || "Subtrahend", SocketTypes.numSocket().valSocket);
        ...
        inp1.addControl(new NumControl((val: number) => this.liveValues.get(node.id)?.set(val), 'num', false, i18n(["subIn"]) || "Minuend"))
        inp2.addControl(new NumControl((val: number) => this.liveValues2.get(node.id)?.set(val), 'num2', false, i18n(["subIn2"]) || "Subtrahend"))
