...
export class ModuloNode extends CompilerNode {
    ...
    async builder(node: RNode): Promise<void> {
        ...
        const inp2 = new Rete.Input('num2', i18n(["modIn2"]) || "Module", SocketTypes.numSocket().valSocket);
        const out = new Rete.Output('num', i18n(["res"]) || "Result", SocketTypes.numSocket().valSocket);

        inp1.addControl(new NumControl((val: number) => this.editor?.trigger("process"), 'num', false, i18n(["divIn"]) || "a"))
        inp2.addControl(new NumControl((val: number) => this.editor?.trigger("process"), 'num2', false, i18n(["divIn2"]) || "Module"))
