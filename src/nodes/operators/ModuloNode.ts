...
export class ModuloNode extends SpreadNode<{ num: number, num2: number }, { res: number }> {
    ...
    async builder(node: RNode): Promise<void> {
        const inp1 = new Rete.Input('num', EditorManager.getInstance()?.i18n(["modIn"]) || "a", SocketTypes.numSocket().valSocket);
        const inp2 = new Rete.Input('num2', EditorManager.getInstance()?.i18n(["modIn2"]) || "Module", SocketTypes.numSocket().valSocket);
        const out = new Rete.Output('num', EditorManager.getInstance()?.i18n(["res"]) || "Result", SocketTypes.numSocket().valSocket);

        inp1.addControl(new NumControl((val: number) => this.editor?.trigger("process"), 'num', false, EditorManager.getInstance()?.i18n(["divIn"]) || "a"))
        inp2.addControl(new NumControl((val: number) => this.editor?.trigger("process"), 'num2', false, EditorManager.getInstance()?.i18n(["divIn2"]) || "Module"))
